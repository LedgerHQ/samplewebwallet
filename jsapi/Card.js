...
var Card = Class.create({
	...
	/**
	 * @class Interface defining the interaction with an ISO7816 object
	 * @property {Number} SW Status Word returned by the last operation as a word
	 * @property {Number} SW1 First byte of the Status Word returned by the last operation
	 * @property {Number} SW2 Second byte of the Status Word returned by the last operation
	 * @constructs
	 */
	initialize : function() {
	 ...
	 */
	exchangeMultiple : function(apdus, statuses, answerAll, throwException) {
	 ...
	 * @returns {ByteString} APDU response without the status word
	 */
	sendApdu : function(cla, ins, p1, p2, opt1, opt2, opt3, wrapScript) {
		var swCheck, le;
		var apduByteString;
		...
		if (cla instanceof ByteString) {
			...
			if (opt1 instanceof ByteString) {
				apdu += Convert.toHexDigit(opt1.length) + opt1.toStringIE(HEX);
			}
			else {
				apdu += "00";
			}
			apduByteString = new ByteString(apdu, HEX);
		...
		if (typeof wrapScript == "undefined") {
			this.response = this.exchange(apduByteString, le);
		}
		else {
			this.response = wrapScript.exchangeWrapped(apduByteString, le);
		}
		if (swCheck) {
			for (var i=0; i<swCheck.length; i++) {
				if (typeof swCheck[i] == "number") {
					if (this.SW == swCheck[i]) {
						break;
					}
				}
				else {
					if ((this.SW & swCheck[i][1]) == swCheck[i][0]) {
						break;
					}
				}
			}
			if (i == swCheck.length) {
				throw "Invalid status " + i + " - " + Convert.toHexShort(this.SW);
			}			
		}		
	 ...
	 */
	_preExchange : function(apduByteString, swCheck, wrapScript) {
	},
